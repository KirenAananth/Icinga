...
execute(){
	...
	TOTAL=`$SCRIPT -e $RETRYCNT -P $VER -t $TMOUT -H $HOST -C $COM_STR -o $RAM_TOTAL | cut -d' ' -f4`
	AVAIL=`$SCRIPT -e $RETRYCNT -P $VER -t $TMOUT -H $HOST -C $COM_STR -o $RAM_AVAIL | cut -d' ' -f4`
	CACHE=`$SCRIPT -e $RETRYCNT -P $VER -t $TMOUT -H $HOST -C $COM_STR -o $RAM_CACHE | cut -d' ' -f4`
	BUFFER=`$SCRIPT -e $RETRYCNT -P $VER -t $TMOUT -H $HOST -C $COM_STR -o $RAM_BUFFER | cut -d' ' -f4`
	if [[ $TOTAL != [0-9]* ]]; then
		ERROR="SNMP Timeout"
		$ECHO "UNKNOWN - $ERROR"
		exit $ST_UK
