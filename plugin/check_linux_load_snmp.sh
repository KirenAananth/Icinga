...
checkload1(){
	RESULT1=`$SCRIPT -e $RETRYCNT -P $VER -t $TMOUT -H $HOST -C $COM_STR -o $Load_1_OID | $CUT -d'"' -f2`
	case $RESULT1 in
		*Timeout*)
			STATUS=UNKNOWN
...
checkload5(){
	RESULT5=`$SCRIPT -e $RETRYCNT -P $VER -t $TMOUT -H $HOST -C $COM_STR -o $Load_5_OID | $CUT -d'"' -f2`
	case $RESULT5 in
		*Timeout*)
			STATUS=UNKNOWN
...
checkload15(){
	RESULT15=`$SCRIPT -e $RETRYCNT -P $VER -t $TMOUT -H $HOST -C $COM_STR -o $Load_15_OID | $CUT -d'"' -f2`
	case $RESULT15 in
		*Timeout*)
			STATUS=UNKNOWN
...
while getopts C:H:w:c:s:q:z: option
	do case "$option" in
		C) COM_STR=$OPTARG;;
		H) HOST=$OPTARG;;
		w) WARN=$OPTARG;;
